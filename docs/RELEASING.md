...
此清单用于维护者发布 Windows x64 NSIS 安装包，以及独立的 macOS arm64 DMG。
...
如有对应平台环境，再运行 `npm run dist:win` 或 `npm run dist:mac:arm64`，
并实际启动对应架构的产物。macOS 构建后还必须运行
`npm run verify:mac:arm64`；严格签名校验失败时不得上传。
...
标签会触发 Release workflow。工作流会再次校验版本，分别构建 Windows x64 与 macOS
arm64 安装包，生成 `SHA256SUMS.txt`，并创建草稿预发布。检查以下项目后再手动发布草稿：

- Windows x64 `.exe` 与 macOS arm64 `.dmg` 均已上传。
- macOS 应用包已通过 `codesign --verify --deep --strict`。
