...
const REMOVED_OPENAI_RELAY_PROVIDER_ID: &str = "codex_tools_openai_relay";
...
pub fn connections_activate_official_account(
    ...
    remove_retired_openai_relay(&mut document);
...
fn remove_retired_openai_relay(document: &mut DocumentMut) {
    normalize_inline_model_providers(document);
    let root = document.as_table_mut();
    if root.get("model_provider").and_then(Item::as_str) == Some(REMOVED_OPENAI_RELAY_PROVIDER_ID) {
        root.remove("model_provider");
    }
    if let Some(providers) = root.get_mut("model_providers").and_then(Item::as_table_mut) {
        providers.remove(REMOVED_OPENAI_RELAY_PROVIDER_ID);
    }
}
...
"#,
    ...
    #[test]
    fn official_activation_removes_retired_openai_relay_configuration() {
        let temp = tempfile::tempdir().unwrap();
        prepare_home(temp.path());
        fs::write(
            temp.path().join("config.toml"),
            "model_provider = \"codex_tools_openai_relay\"\n[model_providers.codex_tools_openai_relay]\nbase_url = \"http://127.0.0.1:43123/retired\"\n[model_providers.other]\nbase_url = \"https://other.example.test/v1\"\n",
        )
        .unwrap();
        let credential = CodexAuthCredential {
            auth_mode: "chatgpt".into(),
        ...
        let config = fs::read_to_string(temp.path().join("config.toml")).unwrap();
        assert!(!config.contains("codex_tools_openai_relay"));
