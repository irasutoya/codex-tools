...
在账号详情页或账号列表的“更多操作”中选择“导出登录凭据”，确认安全提示后保存 JSON 文件。在新电脑中打开该文件，完整复制内容并粘贴到“导入 Cookie”即可导入该账号。

导出文件包含可接管账号的登录凭据：仅通过可信的离线方式转移，导入后立即删除文件，绝不要上传、发送或提交到代码仓库。新电脑刷新登录后，旧电脑的 Refresh Token 可能失效。
